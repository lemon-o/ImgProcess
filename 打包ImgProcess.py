...
# ---------------- 参数 ----------------
NETWORK_DIR = r"\\sa6400\文档\programming\ImgProcess"
LOCAL_DIR = r"C:\virtual environment\ImgProcess"
DIST_DIR = os.path.join(LOCAL_DIR, "dist", "ImgProcess")
DESKTOP_DIR = os.path.join(os.path.expanduser("~"), "Desktop", "ImgProcess")
VERSION_FILE_NAME = "version_info.txt"
PY_FILE_NAME = "ImgProcess.pyw"
ISS_FILE = os.path.join(NETWORK_DIR, "ImgProcess.iss")
ICON_DIR = "icon"

# ---------------- 获取 version_info.txt 当前版本 ----------------
...
# 提取文件中的 FileVersion
match = re.search(r"StringStruct\('FileVersion', '(\d+)\.(\d+)\.(\d+)\.0'\)", content)
if match:
    major, minor, patch = map(int, match.groups())
else:
    print("无法解析 version_info.txt 中的版本号，默认使用 1.0.0")
...
default_ver = f"{major}.{minor}.{patch}"

# ---------------- 弹窗输入版本号 ----------------
app = QApplication(sys.argv)
ver, ok = QInputDialog.getText(None, "输入版本号",
                               "请输入新的版本号（格式如 1.0.0）:",
                               text=default_ver)
...
# ---------------- 复制 ImgProcess.pyw 和 version_info.txt ----------------
...
print(f"已移动 ImgProcess 文件夹到桌面 {DESKTOP_DIR}")
